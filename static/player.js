class Player {
    constructor(name, y, canInteract) {
        ...
        this.handContainer.x = 4 * (cardWidth + 10) + 10;
        this.handContainer.y = y;
        stage.addChild(this.handContainer);
        
        this.deckBitmap = makeCardAt('card_back', 2 * (cardWidth + 10) + 10, this.handContainer.y);
        stage.addChild(this.deckBitmap);
        
        this.discardBitmap = makeCardAt('card_back', 3 * (cardWidth + 10) + 10, this.handContainer.y);
        
        if (canInteract) {
            this.finishButton = document.getElementById('finishButton');
            this.finishButton.style.position = 'absolute';
            this.finishButton.style.display = 'none';
            this.finishButton.style.left = '10px';
    ...
    handleGainedCardToHand(message) {
        if (message.player === this.name) {
            ...
            this.handContainer.addChild(makeCardAt(message.card, this.hand.length * (cardWidth + 10), 0));
        }
    }
    ...
    handleTookCardFromHand(message) {
        if (message.player === this.name) {
            ...
            for (var i = index; i < this.hand.length;i++) {
                this.handContainer.getChildAt(i).x -= cardWidth + 10;
            }
        }
    }
    ...
    handleGainedActions(message) {
        if (message.player === this.name) {
            ...
            if (this.isMyTurn && this.phase === 'ACTION') {
    ...
    addClickListenerToHand(card, onClickFn) {
        for (var i = 0;i < this.hand.length;i++) {
            if (this.hand[i] === card) {
                this.handContainer.getChildAt(i).addEventListener('click', onClickFn);
            }
        }
    }
    
    removeClickListenersFromHand() {
        this.handContainer.children.forEach(cardObj => cardObj.removeAllEventListeners('click'));
    }
